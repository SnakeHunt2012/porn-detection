...
def cut_list(source_list, length, threshold):
    target_list = []
    if len(source_list) <= length:
        source_list.extend([0] * (length - len(source_list)))
        return [source_list]
    while len(source_list) > length:
        target_list.append(source_list[:length])
        source_list = source_list[length:]
    if len(source_list) >= threshold:
        source_list.extend([0] * (length - len(source_list)))
        target_list.append(source_list)
    return target_list
    ...
    with open(pos_data_file, 'r') as fd:
        for line in fd:
            ...
            for word in splited_line:
                if word in word_index_dict:
                    word_index_list.append(word_index_dict[word])
            pos_data_list.extend(cut_list(word_index_list, max_sequence_size, max_sequence_size / 2))
    pos_target_list = [[0.0, 1.0]] * len(pos_data_list)
    ...
    with open(neg_data_file, 'r') as fd:
        for line in fd:
            ...
            for word in splited_line:
                if word in word_index_dict:
                    word_index_list.append(word_index_dict[word])
            neg_data_list.extend(cut_list(word_index_list, max_sequence_size, max_sequence_size / 2))
    neg_target_list = [[1.0, 0.0]] * len(neg_data_list)
